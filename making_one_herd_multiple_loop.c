...
double beta_mm = 0.2  ; //from cow to cow
double beta_mc = 0.01; //from milkers to calves
double rate_sero_conversion = 1/21.0; //mode 21 days 
...
/*===========RESET TABLES IN EACH ITERATION=========================*/
	for(i=0;i<ncol_test_table;i++)
	{
	test_result_table[i]=0;	
	}	
...
//	}
  while (event_day[next_non_markov_date] == NULL)
	 ...
	 //	printf("Type is %d",current_event->event_type) ;
	 	...
	 	{
	 		//this is to remove events for animals that do not exist anymore
 ...
 //printf("Before update");
  ...
  dry_season,milk_numbers, event_day) ;
//printf("After update");
...
//	printf("Determining sample size for group%d\n",i);
	...
	else if(i>=id_lact_group && target_adult>0)
	{
		...
		{
			if(target_slaughter_adult+target_live_adult<=List_mng_status[i][0])
			{
...
for(i=0; i<=id_dry_group; i++)
{
//printf("sampling from %d",i);
	//TASK_37
N_PPS = 0; //number of remaining previous positive or sick animals
...
//printf("S1");
	...
	a = counter_slaughter_pcr_nomilk;
	b = counter_slaughter_ELISA;
...
while(current_animal!=NULL)
{
	...
	if(counter_slaughter<GroupSampleSize[i][slaughter])
	{
			//still slaughter samples
				...
				Se_bio, Sp_bio, Se_ID, Sp_ID, PCR_live, PCR_slaughter) ;
				current_animal->present = 0;
				}
...
while(current_animal!=NULL)//Round 1 of sampling slaughter: sick animals
	{
	...
	if(current_animal->mbovis_elisa_status==1||current_animal->mbovis_pcr_status==1||current_animal->mastitis_detected==1)
	{
		...
		//	printf("N_PPS is %d", N_PPS) ;
			...
			List_mng_status[i][0] --;
				if(List_mng_status[i][0]<0)
			{
				printf("(C)Now group %d goes minus\n",List_mng_status[i][0]);
				system("pause");
			}
			List_mng_status[i][mbovis_status+5] --;
					if(*dry_season==0 && group ==1)
	...
	}//Round 1: slaughter sampling done
	...
	//	printf("counter_slaughter_ELISA is %d\n",counter_slaughter_ELISA);
		...
		//	printf("N_H is %d", N_H) ;
			...
			List_mng_status[i][0] --;
				if(List_mng_status[i][0]<0)
			{
				printf("(D)Now group %d goes minus\n",List_mng_status[i][0]);
				system("pause");
			}
			List_mng_status[i][mbovis_status+5] --;
					if(*dry_season==0 && group ==1)
...
if(current_event->event_type==16)
   {//TASK_41
	...
	random_number--;
		...
		List_mng_status[group][column_Exp]++;
...
/*=================EVENT 16: Risk Event done====================*/
    ...
    for(i=0;i<id_bull_group+1;i++)
	{
	//	printf("A: number is %lf",List_mng_status[i][0]);
...
for(i=0;i<length_animal_pointer;i++)
{
...
int* num_bobby, int* dry_season, int* milk_numbers, struct event_node* event_day[])
{
...
/// TASK_2 deleted
	if(i==0 && *dry_season==0 && List_mng_status[id_calf_group][0]>0)
		{
...
//TASK_5 deleted and modified
		...
		*List_mng_status[i][5];
		if(List_mng_status[i][9]<0)
		{
			printf("milk_numbers 4 is %d",milk_numbers[4]);
			printf("milk_numbers 7 is %d",milk_numbers[7]);
			system("pause");
		}
...
//	printf("V is %f\n",random_value-List_mng_status[id_lact_group2][15]-demographic_rate) ;
	if(random_value-List_mng_status[id_lact_group2][15]-demographic_rate<=random_value*0.0001) //CM incidnece
	/*==============1. CM incidence===================================*/
	{
//		printf("CM incidence");
		//TASK_7 completed
...
/*====================CM DETECTION========================================*/
	else if(random_value- List_mng_status[id_lact_group2][16]-List_mng_status[id_lact_group2][15]-demographic_rate<=random_value*0.0001)
	{
//	printf("CM detection");
	animal_to_chose = 	FarmGroupList[id_lact_group2];
		//Assumption_2: 
...
/*====================CM DETECTION DONE================================*/
	...
	else if(random_value- List_mng_status[id_lact_group2][16]-List_mng_status[id_lact_group2][15]-demographic_rate-disease_transition_sum<=random_value*0.0001)
/*====================MBOVIS STATUS TRANSITION=============================*/
	{

//	printf("MBOVIS transition");
...
//	printf("S to E\n");
	...
	else if(random_value<List_mng_status[i-1][column_r_SE]+List_mng_status[i-1][column_r_SE+1])
	{
//	printf("E to Is\n");
	...
	else if(random_value<List_mng_status[i-1][column_r_SE]+List_mng_status[i-1][column_r_SE+1]+List_mng_status[i-1][column_r_SE+2])
	{
//	printf("Is to Ic\n");
	...
	else if(random_value<List_mng_status[i-1][column_r_SE]+List_mng_status[i-1][column_r_SE+1]+List_mng_status[i-1][column_r_SE+2]+List_mng_status[i-1][column_r_SE+3])
	{
//	printf("Ic to IsE\n");
	...
	List_mng_status[i-1][column_r_SE+2]+List_mng_status[i-1][column_r_SE+3] +List_mng_status[i-1][column_r_SE+4] )
	{
//	printf("Is to E\n");
